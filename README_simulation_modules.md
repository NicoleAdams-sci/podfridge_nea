...
> **Focal ranking test.** A supplementary workflow builds on this pipeline to ask
> a database-search question: dropped into a database of unrelated individuals,
> does a true relative rank near the top? See
> [Focal Ranking Test](#focal-ranking-test) below for the overview, and
> [`README_focal_ranking_test.md`](README_focal_ranking_test.md) for the full
...
│   ├── run_statistical_tests.R              # All inferential tests
│   ├── run_statistical_tests.sh             # SLURM wrapper for stats
...
│   ├── module_test_usage.R      # Dev scratch script exercising modules 1-8 interactively
│   │
│   │   # --- Focal ranking test (see README_focal_ranking_test.md) ---
│   ├── generate_unrelated_pool.R/.sh        # Unrelated database generation (wraps module 8)
│   ├── focal_test_helper_fns.R              # Database assembly from existing pairs
│   ├── module4_single_locus_LR_fast.R       # Vectorized single-locus LR (focal only)
│   ├── module10_ranking_lr_calculator_fast.R  # Focal-vs-database combined LRs
│   ├── module11_ranking_outcome_recorder.R    # Rank candidates, record top-N outcomes
│   ├── make_focal_ranking_manifest.sh       # Pair/LR/pool manifest builder
│   ├── submit_focal_ranking.sh              # SLURM array driver
│   ├── run_focal_ranking.R                  # Rank one chunk of pair IDs
│   ├── combine_ranking_outcomes.sh          # Concatenate per-chunk outcomes
│   ├── plot_ranking_summary.R               # Per-database figures
│   └── compare_ranking_across_databases.R   # Cross-database figures
│
├── analysis/                # R Markdown analysis scripts and reports (Tina)
│   ├── simulation_analysis.Rmd       # Statistical analysis R Markdown
│   ├── simulation_analysis_*.html    # Generated HTML reports (dated)
│   ├── focal_ranking_test.Rmd        # Focal ranking results report
│   └── focal_ranking_test.html       # Rendered focal ranking report
│
...
│   └── sim_pairs_array_assignments.csv  # Array ID -> (population, relationship, chunk)
│
├── tests/                   # testthat unit + integration tests (Tina)
│   ├── testthat.R
│   └── testthat/
│
├── code_graveyard/          # Archived/superseded code (gitignored for new files)
│
...
└── output/                  # Generated data and results
    ...
    └── lr_analysis_*/       # Analysis results and plots
        ...
        ├── locus_inflation/                 # Per-locus inflation analysis outputs
        ...
        │   └── plots_unordered/             # Superseded exploratory figures; kept
        │                                    # for reference, not regenerated by
    ...
    # --- Focal ranking test outputs (see README_focal_ranking_test.md) ---
    ├── unrelated_pool/      # Unrelated databases (gitignored - large)
    ├── focal_test_<size>/   # One dir per database size: manifest, chunks/,
    │                        # combined outcomes, ranking_summary_statistics.csv, figures/
    └── focal_cross_db/      # Cross-database comparison figures + summary CSVs
```
...
Focal Ranking Test (supplementary — see README_focal_ranking_test.md):
└── module8_unrelated_pool_generator.R        (builds the unrelated database)
    └── focal_test_helper_fns.R               (assembles focal vs. database)
        └── module10_ranking_lr_calculator_fast.R
            └── module11_ranking_outcome_recorder.R
```

**On module numbering**: the numbers are identifiers, not a pipeline order.
Modules 6 and 7 are not on any live path; module 9 is post-processing; modules
10–11 belong to the supplementary focal test. An earlier focal draft numbered
these 10/11/12, but the standalone database assembler was folded into
`focal_test_helper_fns.R`, so the remaining two were renumbered 11→10 and 12→11
(August 2026).
...
6. **module6_single_combo_pair_generator.R**: Batch pair generation wrapper
   ...
   - **Status: not on any live path.** Superseded by direct `sim_pairs.R` calls.
     Only remaining references are `code/module_test_usage.R` and a commented-out
     `source()` at `code/sim_pairs.R:28`.
...
7. **module7_single_pop_focal_generator.R**: Family structure simulator
   ...
   - **Status: not on any live path — and not pending implementation either.**
     This module simulates *fresh* focal families. The focal ranking test that was
     actually built takes the opposite approach: it reuses individuals from the
     existing `pairs_*.csv` files, so it never calls module 7. The module is
     retained, working and unit-tested
     (`tests/testthat/test-module7-focal-generator.R`), in case a
     simulate-fresh-families design is revisited. The archived usage recipe lives
     at `code_graveyard/focal_family_generation/`.
...
8. **module8_unrelated_pool_generator.R**: Unrelated individual pool generator
   ...
   - **Status: in production.** This is the database generator for the focal
     ranking test. `code/generate_unrelated_pool.R` sources it (line 186) and calls
     `generate_unrelated_pool()` (line 320) once per population chunk, then relabels
     IDs globally so mixed-population pools don't collide. Driven by
     `generate_unrelated_pool.sh`; step 1 of the focal workflow.
...
#### Focal Ranking Modules

These two belong to the supplementary focal ranking test. Summarized here for
completeness; documented in full in
[`README_focal_ranking_test.md`](README_focal_ranking_test.md).

10. **module10_ranking_lr_calculator_fast.R**: Focal-vs-database LR calculator
    - Dependencies: `focal_test_helper_fns.R`, modules 1–5, and
      `module4_single_locus_LR_fast.R`
    - Function: `calculate_ranking_lrs()` / `calculate_ranking_lrs_replicates()`
    - Computes a combined LR for every focal-vs-candidate pair, under each tested
      hypothesis × loci set
    - Uses `module4_single_locus_LR_fast.R` — a vectorized `data.table`-join
      rewrite of module 4 (~10–100× faster) that leaves the original module 4
      untouched. **Validated for `parent_child` and `full_siblings` only.**

11. **module11_ranking_outcome_recorder.R**: Ranking and outcome recorder
    - Dependencies: module 10 output
    - Function: `record_ranking_outcomes()`
    - Ranks all database members by combined LR within each replicate, finds the
      true relative's rank, and records top 10/50/100/200 flags plus tied-group size
...
> scripts designed for manuscript figures, and are no longer in `code/`
> (`plots_matched.R` is archived in `code_graveyard/`). The new scripts use the
...
19. **plots_cutoffs_publication.R** *(replaces plots_proportion_exceeding_cutoffs.R)*: Classification performance figures
    ...
    - Dependencies: tidyverse, data.table, scales, patchwork
    - Figures:
      - **Figure 1 — Main text** (`cutoff_classification_0.01fpr_core13_expanded20.png`): Grouped
        bar chart of proportion of pairs exceeding the 0.01% FPR threshold, contrasting the
        **Core 13 and Expanded 20** panels; faceted by tested hypothesis + loci set (rows,
        so the two loci conditions sit adjacent) × true relationship (columns); colored by
        population; fixed y-axis for direct comparability; 18 × 16 inches
        - An alternative stacked A/B layout (one patchwork panel per loci set, free y-axes,
          output `cutoff_classification_0.01fpr_core13_expanded20_AB.png`) is preserved
          commented-out in Section 3 of the script. Swap the two blocks to use panel-letter
          referencing instead of facet rows.
...
25. **run_pipeline_bare.sh**: Manual step-by-step command reference
    ...
    - Covers Steps 1–7 of the main pipeline; the focal ranking test has its own
      command reference in `README_focal_ranking_test.md`

#### Development Scratch Script
26. **module_test_usage.R**: Interactive smoke-test of modules 1–8
    - Not part of any pipeline; sources modules 1–8 and calls each generator once
      with small inputs to confirm they still run and return the expected shape
    - **Caveat**: hardcodes `setwd("~/Documents/lasisiLab/podfridge_nea/")` near the
      top, so it will not run as-is on another machine or from another checkout
    - For actual regression coverage, use the `tests/` suite below rather than this

## Tests

`testthat` unit and integration tests live in `tests/testthat/`. Run the whole
...
| Test file | Covers |
|---|---|
| `test-count-shared-alleles.R` | `count_shared_alleles()` — IBD allele counting with multiplicity |
| `test-label-and-genotype.R` | `label_and_genotype()` — deterministic allele→letter mapping |
| `test-calculate-likelihood-ratio.R` | `calculate_likelihood_ratio()` — the core LR formula (pure math, no data) |
| `test-kinship-calculation.R` | `kinship_calculation()` — the three pure functions plus frequency/coefficient lookup |
| `test-lr-all-relationships.R` | LR values across all relationship types × all genotype patterns |
| `test-calculate-combined-lr.R` | `calculate_combined_lr()` — multiplying single-locus LRs across loci sets |
| `test-integration-kinship-pipeline.R` | End-to-end `kinship_calculation()` behavior |
| `test-module7-focal-generator.R` | Module 7 focal family generation (see the module 7 status note above) |
| `test-module9-stats-functions.R` | Module 9 summary/ratio/cutoff statistics |

`tests/testthat/helper-focal_module.R` provides the module 7 fixtures: a
path-resolving `source_module_file()` that works whether tests run from the repo
root or from `tests/testthat/`, mock allele-frequency and kinship tables, and
`with_mocked_focal_functions()` for stubbing the simulators.

`tests/testthat.R` is the bootstrap file and documents the same runner command.
...
**Array ID mapping**: `docs/sim_pairs_array_assignments.csv` lists which
(population, relationship, chunk) each `sim_pairs.sh` array task ID corresponds
to — useful when a single task fails and you need to resubmit just that one.
...
- `cutoff_classification_0.01fpr_core13_expanded20.png` (main text, single threshold, Core 13 vs Expanded 20)
...
A supplementary workflow that reframes the same data as a **database search**.
Full details — file-by-file roles, the complete command sequence, and a
conventions/gotchas section — are in
[`README_focal_ranking_test.md`](README_focal_ranking_test.md). This is the
orientation summary.

### The question

Take individual 1 of an already-simulated pair as a "focal" query profile and
individual 2 as its true relative. Drop the true relative into a large database
of unrelated individuals, score every database member against the focal profile
by combined LR, rank them, and ask:

> Does the true relative land in the top N (200 / 100 / 50 / 10)?

Recovery is evaluated across **true relationship** (parent–child, full siblings,
half siblings), **tested hypothesis** (every pair scored under both
`parent_child` and `full_siblings`, regardless of truth), **loci set** (Core 13
vs. Expanded 20), and **database size** (e.g. 10k / 20k / 100k). Each existing
pair is one replicate, so 10 chunks × 1,000 pairs = 10,000 replicates per true
relationship.

### How it reuses the main pipeline

The focal test resimulates nothing:

- **Focal + true relative** come from the existing `output/pairs_*.csv` files.
...
- **Modules 1–5, module 8, and `LR_kinship_utility_functions.R`** are shared unchanged.

What the focal layer adds: unrelated-database generation
(`generate_unrelated_pool.R`, wrapping module 8), database assembly
(`focal_test_helper_fns.R`), a vectorized LR path
(`module4_single_locus_LR_fast.R` → `module10_ranking_lr_calculator_fast.R`),
the ranking/outcome recorder (`module11_ranking_outcome_recorder.R`), SLURM
orchestration, and the plotting scripts.

### Workflow shape

Steps 1–5 run **once per database size**; steps 6–7 run **once**, across all sizes.

| Step | Script | Does |
|---|---|---|
| 1 | `generate_unrelated_pool.sh` | Build the unrelated database for size N |
| 2 | `make_focal_ranking_manifest.sh` | Match pair files + combined_LR files + this pool |
| 3 | `submit_focal_ranking.sh` | SLURM array: rank each chunk of pairs against the pool |
| 4 | `combine_ranking_outcomes.sh` | Concatenate per-chunk outcomes into one CSV |
| 5 | `plot_ranking_summary.R` | Per-database figures + summary table |
| 6 | `compare_ranking_across_databases.R` | Cross-database recovery curves and degradation |
| 7 | `analysis/focal_ranking_test.Rmd` | Knit everything into one HTML report |

Outputs land in `output/unrelated_pool/`, `output/focal_test_<size>/`, and
`output/focal_cross_db/`. Only the small artifacts (figures, summary CSVs) are
committed; pools and per-chunk outcomes are gitignored and regenerable.

### Two things to know before reading results

- **Top-N recovery is the primary metric, not median rank.** Under a mismatched
  hypothesis, most candidates hit a no-sharing locus and collapse to
  `combined_LR = 0`; `rank(ties.method = "average")` then hands that whole block
  one averaged rank ≈ `n_database / 2`. Median rank in those cells is an
  averaging artifact.
- **The fast single-locus LR was validated for `parent_child` and
  `full_siblings` only.** Re-validate `module4_single_locus_LR_fast.R` against
  `module4_single_locus_LR.R` before extending to other tested hypotheses.
