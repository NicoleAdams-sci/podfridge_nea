...
ls output/pairs_*.csv > lr_file_list.txt 2>/dev/null

# Check if any files were found
if [ ! -s lr_file_list.txt ]; then
...
# Count files
FILE_COUNT=$(wc -l < lr_file_list.txt)
echo "Found $FILE_COUNT pairs files to process:"

# Show first few files as preview
head -5 lr_file_list.txt
...
JOB_ID=$(sbatch submit_lr_jobs_ready.sh | cut -d' ' -f4)
