# ------------------------------------------------------------------------------
# Module 10: Ranking Test LR Calculator
# ------------------------------------------------------------------------------
#
# Purpose:
...
source("code/module4_single_locus_LR_fast.R")
source("code/module5_combined_LR.R")


#' Calculate combined LRs for a single ranking search database
#'
#' Takes one assembled paired database (from focal_test_helper_fns.R) and
#' calculates combined LRs across all loci sets for the specified tested
#' relationships and population frequencies.
#'
#' @param paired_db Data frame, output from
#'        assemble_unrelated_database_from_existing_pair() in focal_test_helper_fns.R.
...
calculate_ranking_lrs <- function(paired_db,
  ...
  single_locus_all <- map_dfr(tested_relationships, function(rel) {
    cat(sprintf("  Running single locus LR (fast/vectorized) for tested relationship: %s\n", rel))
    calculate_single_locus_lr_fast(
      pair_data            = paired_db,
...
#'
#' @param replicate_dbs Named list of paired data frames assembled by
#'        focal_test_helper_fns.R, one per replicate.
#' @param tested_relationships Character vector, relationships to test as H1.
#' @param tested_populations Character vector, population frequencies to use.
...
#   paired_db             = paired_db,         # from focal_test_helper_fns.R
#   tested_relationships  = c("parent_child", "full_siblings"),
#   tested_populations    = "all",
#   loci_sets             = loci_sets,
...
#   replicate_dbs         = replicate_dbs,     # from focal_test_helper_fns.R
#   tested_relationships  = c("parent_child", "full_siblings"),
#   tested_populations    = "all",
#   loci_sets             = loci_sets,
#   allele_frequency_data = df_allelefreq,
